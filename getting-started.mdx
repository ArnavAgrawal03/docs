...
<Tabs>
  ...
  <Tab title="Self Host - Docker (Advanced)">
    ...
    **Option&nbsp;A — Run the pre-built image**
    ...
    This is the recommended method for most users. It uses `docker-compose` to run the pre-built Morphik image along with its required Postgres and Redis containers.

    <Steps>
    <Step title="Download the Compose file">
      You'll need a `docker-compose.run.yml` file to tell Docker how to run Morphik and its dependencies.

      ```bash
      # Download the file from the Morphik repository
      curl -o docker-compose.run.yml https://raw.githubusercontent.com/morphik-org/morphik-core/main/docker-compose.run.yml
      ```
    </Step>

    <Step title="Create your Environment file">
      Create a `.env` file in the same directory. This is where you will put your API keys.
      ...
      # A secret key for signing JWTs. Change this in a production environment.
    ...
    <Step title="Start the application">
      Now, you can start all the services with a single command.

      ```bash
      # This command will pull the necessary images and start the containers in the background
      docker compose -f docker-compose.run.yml up -d
      ```
      The first time you run this, it may take a few minutes to download the images. Subsequent starts will be much faster. The server will be available at [http://localhost:8000](http://localhost:8000).
    </Step>
    </Steps>

    <Accordion title="How to customize your configuration (Optional)">
      The default `docker compose up` command uses the configuration baked into the Morphik image. If you need to change models, embedding dimensions, or other advanced settings, follow these steps:

      1.  **Extract the default configuration:** Run this command to get a local copy of the `morphik.toml` file.
      ...
      2.  **Edit the local `morphik.toml` file:** Open the file you just created and make your desired changes.

      3.  **Enable the custom config in `docker-compose.run.yml`:** Open `docker-compose.run.yml` in a text editor and uncomment the volume mount for both the `morphik` and `worker` services.
      ...
      4.  **Start the application:** Run `docker compose -f docker-compose.run.yml up -d`. It will now use your local `morphik.toml` file.
    </Accordion>

    <Accordion title="How to use local models with Ollama (Optional)">
    To use local models instead of OpenAI, start the services with the `ollama` profile enabled:

      ```bash
      docker compose -f docker-compose.run.yml --profile ollama up -d
      ```

    Make sure your `morphik.toml` is configured to use Ollama models. The default `api_base` for Ollama in the compose file is `http://ollama:11434`.
    </Accordion>
