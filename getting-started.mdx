  ...
  <Tab title="Self Host - Docker (Advanced)">
    ...
    **Option&nbsp;A — Run the pre-built image**
    ...
    <Accordion title="How to customize your configuration (Optional)">
      If you need to change the models, embedding dimensions, or other advanced settings, follow these steps:

      1.  **Extract the default configuration:** Before starting the services, run this command to get a copy of the default `morphik.toml` file.
      ...
      2.  **Edit the file:** Open the `morphik.toml` file you just created and make your desired changes.

      3.  **Start the application:** Run the `docker compose up` command as shown above. The `docker-compose.run.yml` file is configured to automatically detect and use your local `morphik.toml`, overriding the internal default.
    </Accordion>
