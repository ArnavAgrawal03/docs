...
<Tabs>
  ...
  <Tab title="Self Host - Docker (Advanced)">
    ...
    **Option&nbsp;A — Run the pre-built image**
    ...
    docker pull ghcr.io/morphik-org/morphik-core:latest

    # 2. Create an env file with your secrets – at minimum OPENAI_API_KEY
    cat > morphik.env << 'EOF'
    OPENAI_API_KEY=sk-...
    # JWT_SECRET_KEY=change-me-in-prod  # optional, defaults internally
    EOF

    # 3 (optional).  Grab the default config so you can tweak it locally
    #    The image ships the file at /app/morphik.toml.default
    #    Copy it once, edit in your editor, then mount it back in step 4.
    docker run --rm ghcr.io/morphik-org/morphik-core:latest \
               cat /app/morphik.toml.default > morphik.toml

    # 4. Launch the container – mount the config & env file
    #    If you did not tweak the file, omit the -v flag and the image will
    #    use its internal defaults (OpenAI models, 1536-d pgvector table).
    docker run -d --name morphik \
      --env-file morphik.env \
      -v $(pwd)/morphik.toml:/app/morphik.toml:ro \
      -p 8000:8000 ghcr.io/morphik-org/morphik-core:latest
    ```

    The server is now running at [http://localhost:8000](http://localhost:8000).

    **Changing models / behaviour later**  
    Just edit `morphik.toml` and `docker restart morphik`.  Any field you omit keeps the baked-in default.
    ...
    **Option&nbsp;B — Develop from source (build locally)**
    ...
    1. Generate `.env` if missing (includes placeholders for `OPENAI_API_KEY`, etc.)
    2. Copy **morphik.docker.toml ➜ morphik.toml** (overwriting each run) so the stack
       always uses the OpenAI config.
