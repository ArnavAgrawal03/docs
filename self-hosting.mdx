...
<Tabs>
  ...
  <Tab title="Self Host - Docker (Advanced)">
    ...
    **Option&nbsp;A — Using Pre-built Image (Recommended)**
    ...
    The script handles all setup tasks including checking prerequisites, downloading configurations, and starting services. After installation:

    ```bash
    # To restart Morphik later
    ./start-morphik.sh
    ...
    The server will be available at the port specified in your `morphik.toml` configuration (default: 8000).
